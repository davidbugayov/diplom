...
package io.myMesh.util.nslogger;
...
public class NSLogger
{
	...
	{
		if (debugLogger)
			Log.v("myMesh", "NSLogger Pushing client info to front of queue");
	...
	private void connectImpl()
	{
		...
		{
			Log.w("myMesh", String.format("nslogger connect failed (unknown host) to %s:%d", host, port));
			NSLogger.this.state = State.CLOSED;
			return;
		}
		catch (IOException ex)
		{
			Log.w("myMesh", "nslogger connect failed to " + host + ":" + port, ex);
			NSLogger.this.state = State.CLOSED;
			return;
		}

		Log.v("myMesh", "nslogger base socket connected");
		...
		if(useSSL)
		{
			...
			{
				Log.w("myMesh", "nslogger ssl socket failed to " + host + ":" + port, ex);
				NSLogger.this.state = State.CLOSED;
				try
				{
					socket.close();
